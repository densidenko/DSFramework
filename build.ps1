...
exec { & dotnet build DSFramework.sln -c Release /p:Version=$buildVer }
...
ForEach ($folder in $testDirs) { 
    ...
    exec { & dotnet test $folder.FullName -c Release --no-build --no-restore /p:CollectCoverage=true /p:CoverletOutput=$root\coverage /p:MergeWith=$root\coverage.json /p:Include="[*]DSFramework.*" /p:Exclude="[*]DSFramework.Testing.*" $format }
}
