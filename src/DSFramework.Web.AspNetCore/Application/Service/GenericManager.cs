...
using DSFramework.Web.AspNetCore.Extensions;
using Microsoft.Extensions.Logging;
using Polly;

namespace DSFramework.Web.AspNetCore.Application.Service
{
    public abstract class
        GenericManager<TEntity, TRepository> : GenericManager<string, TEntity, EntityHolder<TEntity>, WriteCommand<TEntity>, TRepository>
    ...
    public abstract class
        GenericManager<TKey, TEntity, TRepository> : GenericManager<TKey, TEntity, EntityHolder<TKey, TEntity>, WriteCommand<TKey, TEntity>,
            TRepository> where TRepository : IRepository<TKey, EntityHolder<TKey, TEntity>>
    {
        protected GenericManager(ILoggerFactory loggerFactory, TRepository repository, IManagerObserver observer)
            : base(loggerFactory, repository, observer)
        { }
    ...
    public abstract class
        GenericManager<TKey, TEntity, TEntityHolder, TWriteSnapshotCommand, TRepository> : ReadOnlyGenericManager<TKey, TEntityHolder, TRepository>,
                                                                                           IGenericManager<TKey, TEntity, TEntityHolder,
                                                                                               TWriteSnapshotCommand>, IDisposable
        ...
        where TEntityHolder : EntityHolder<TKey, TEntity>, new()
    {
        ...
        public virtual async Task<BulkWriteResult<TKey>> BulkUpdate(TWriteSnapshotCommand[] commands)
            => await Observe(nameof(BulkUpdate), async () => await BulkUpdateInternal(commands));

        public virtual async Task<TKey> Update(TWriteSnapshotCommand command)
            => await Observe(nameof(Update), async () => await UpdateInternal(command));

        public virtual async Task<TEntityHolder> Delete(TKey entityId) => await Observe(nameof(Delete), async () => await DeleteInternal(entityId));
        ...
        protected void RaiseEntityChanged(TEntityHolder entityHolder) => EntityChanged?.Invoke(entityHolder);

        protected void RaiseEntityDeleted(TEntityHolder entityHolder) => EntityDeleted?.Invoke(entityHolder);
