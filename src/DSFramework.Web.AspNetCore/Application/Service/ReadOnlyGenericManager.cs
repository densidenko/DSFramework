...
namespace DSFramework.Web.AspNetCore.Application.Service
{
        ...
        where TRepository : IRepository<TKey, TEntity>
    {
        ...
        public virtual async Task<TEntity> Get(TKey id) => await Observe(nameof(Get), async () => await Repository.GetAsync(id));

        public virtual async Task<TEntity[]> GetMany(TKey[] ids) => await Observe(nameof(GetMany), async () => await Repository.GetManyAsync(ids));

        public virtual async Task<TEntity[]> GetAll() => await Observe(nameof(GetAll), async () => await Repository.GetAllAsync());

        public virtual async Task<TEntity[]> Search(IDomainSpecification<TEntity> specification)
            => await Observe(nameof(Search), async () => await Repository.SearchAsync(specification));

        protected virtual async Task<T> Observe<T>(string actionName, Func<Task<T>> action)
            => await Observer.Observe<TEntity, T>(actionName, action);

        protected async Task Observe(string actionName, Func<Task> action) => await Observer.Observe<TEntity>(actionName, action);
    }
}