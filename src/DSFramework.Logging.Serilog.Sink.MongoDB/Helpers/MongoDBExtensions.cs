...
namespace DSFramework.Logging.Serilog.Sink.MongoDB.Helpers
{
    public static class MongoDBExtensions
    {
        ...
        internal static void VerifyCollectionExists(this IMongoDatabase database,
            ...
            var model =
                new
                    CreateIndexModel<BsonDocument>(Builders<BsonDocument>.IndexKeys.Combine(Builders<BsonDocument>
                                                                                            ...
                                                                                            Builders<BsonDocument>
                                                                                                .IndexKeys.Ascending(RequestProperties
                                                                                                                         .MESSAGE_TEMPLATE)),
            ...
            var model =
                new
                    CreateIndexModel<BsonDocument>(Builders<BsonDocument>.IndexKeys.Combine(Builders<BsonDocument>
                                                                                            ...
                                                                                            Builders<BsonDocument>
                                                                                                .IndexKeys
                                                                                                .Ascending($"{RequestProperties.EVENT_ID}.Id"),
                                                                                            Builders<BsonDocument>
        ...
        {
            var model =
                new CreateIndexModel<BsonDocument>(Builders<BsonDocument>
                                                   .IndexKeys.Hashed($"{RequestProperties.PROPERTIES}.{RequestProperties.IP_ADDRESS}"),
                                                   new CreateIndexOptions { Background = true });
            collection.Indexes.CreateOneAsync(model);
        }
        ...
        private static void EnsureHashedIndexOnCorrelationId(IMongoCollection<BsonDocument> collection)
        {
            var model =
                new CreateIndexModel<BsonDocument>(Builders<BsonDocument>
                                                   .IndexKeys.Hashed($"{RequestProperties.PROPERTIES}.{RequestProperties.CORRELATION_ID}"),
                                                   new CreateIndexOptions { Background = true });
            collection.Indexes.CreateOneAsync(model);
        }
        ...
        private static void EnsureHashedIndexOnRequestId(IMongoCollection<BsonDocument> collection)
        {
            var model =
                new CreateIndexModel<BsonDocument>(Builders<BsonDocument>
                                                   .IndexKeys.Hashed($"{RequestProperties.PROPERTIES}.{RequestProperties.REQUEST_ID}"),
                                                   new CreateIndexOptions { Background = true });
            collection.Indexes.CreateOneAsync(model);
        }
        ...
        private static void EnsureHashedIndexOnConnectionId(IMongoCollection<BsonDocument> collection)
        {
            var model =
                new CreateIndexModel<BsonDocument>(Builders<BsonDocument>
                                                   .IndexKeys.Hashed($"{RequestProperties.PROPERTIES}.{RequestProperties.CONNECTION_ID}"),
                                                   new CreateIndexOptions { Background = true });
            collection.Indexes.CreateOneAsync(model);
        }
    }
