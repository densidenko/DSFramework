...
using DSFramework.Data.MongoDB.Contexts;
using DSFramework.Data.MongoDB.Specifications;
using DSFramework.Data.MongoDB.Specifications.Converter;
...
namespace DSFramework.Data.MongoDB
{
    public abstract class BaseMongoRepository<TKey, TAggregate> : BaseMongoRepository<TKey, TKey, TAggregate, TAggregate>
        where TKey : IComparable<TKey> where TAggregate : IHasId<TKey>
    {
        private class FakeDocumentConverter : IConverter<TAggregate, TAggregate>
        {
            ...
            public TAggregate Convert(TAggregate source) => source;
        }
        ...
        private class FakeKeyConverter : IConverter<TKey, TKey>
        {
            ...
            public TKey Convert(TKey source) => source;
        }
    ...
    public abstract class
        BaseMongoRepository<TDomainKey, TDataKey, TAggregateRoot, TAggregateRootData> : IMongoRepository, IRepository<TDomainKey, TAggregateRoot>
        where TDomainKey : IComparable<TDomainKey> where TAggregateRootData : IHasId<TDataKey>
    {
        ...
        protected virtual void DoCreateIndices(IMongoCollection<TAggregateRootData> collection,
        ...
        public TAggregateRoot[] GetAll()
        {
            using (Logger.CreateTimeGuard())
            {
                return Observe(() => SearchMongo(null, null, out _, SearchOptions.NoLimits),
        ...
        protected IMongoCollection<TAggregateRootData> GetOperativeCollection() => GetCollection<TAggregateRootData>();

        protected IMongoCollection<TDocument> GetCollection<TDocument>(string partitionKey = null)
            => DbContext.GetCollection<TDocument>(partitionKey);
        ...
        private FilterDefinition<TAggregateRootData> FindByKey(TDomainKey key) => _filterBuilder.Eq(data => data.Id, KeyConverter.Convert(key));

        private FilterDefinition<TAggregateRootData> FindByKey(TDataKey key) => _filterBuilder.Eq(data => data.Id, key);
        ...
        private static bool IsUpdating(IHasVersion document) => document.DataVersion > Constants.INITIAL_DATA_VERSION;

        private SearchResult<TAggregateRoot> SearchMongo(IMongoSpecification<TAggregateRootData> specification,
                                                         ...
                                                         SearchOptions searchOptions)
        {
            ...
            {
                hasMore = false;
                docs = collection.Aggregate(new AggregateOptions { AllowDiskUse = true }).Match(filter).ToList();
            ...
            else
            {
                ...
                docs = collection.Aggregate(new AggregateOptions { AllowDiskUse = true }).Match(filter).Limit(biggerLimit).ToList();
                result = docs.Select(doc => Tuple.Create(doc, Converter.Convert(doc))).ToArray();
        ...
        public Task<TAggregateRoot> GetAsync(TDomainKey key) => throw new NotImplementedException();

        public Task<TAggregateRoot> UpdateAsync(TAggregateRoot entity) => throw new NotImplementedException();

        public Task<TAggregateRoot[]> GetAllAsync() => throw new NotImplementedException();

        public TAggregateRoot[] GetMany(params TDomainKey[] keys) => throw new NotImplementedException();

        public Task<TAggregateRoot[]> GetManyAsync(params TDomainKey[] keys) => throw new NotImplementedException();

        public void BulkUpsert(TAggregateRoot[] documents) => throw new NotImplementedException();

        public Task BulkUpsertAsync(TAggregateRoot[] entities) => throw new NotImplementedException();

        public TAggregateRoot Delete(TDomainKey key) => throw new NotImplementedException();

        public Task<TAggregateRoot> DeleteAsync(TDomainKey key) => throw new NotImplementedException();

        public TAggregateRoot[] DeleteMany(TDomainKey[] keys) => throw new NotImplementedException();

        public Task<TAggregateRoot[]> DeleteManyAsync(TDomainKey[] keys) => throw new NotImplementedException();

        public TAggregateRoot[] DeleteMany(IDomainSpecification<TAggregateRoot> specification) => throw new NotImplementedException();

        public Task<TAggregateRoot[]> DeleteManyAsync(IDomainSpecification<TAggregateRoot> specification) => throw new NotImplementedException();

        public long Count(IDomainSpecification<TAggregateRoot> specification) => throw new NotImplementedException();

        public long Count(IDomainSpecification<TAggregateRoot> specification, SearchOptions searchOptions) => throw new NotImplementedException();
        ...
        public Task<long> CountAsync(IDomainSpecification<TAggregateRoot> specification) => throw new NotImplementedException();
