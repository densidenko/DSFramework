...
using DSFramework.Data.Caching;
...
namespace DSFramework.Data.MongoDB
{
        ...
        where TDocument : IHasId<TKey> where TKey : IComparable<TKey>
    {
        ...
        protected CachedMongoRepository(IMongoDbContext mongoDbContext,
        ...
        protected override IMongoCollection<TDocument> GetOperativeCollection() => DbContext.Database.GetCollection<TDocument>(CollectionName);
    }
        ...
        where TDocument : IHasId<TKey> where TKey : IComparable<TKey>
    {
        ...
        private CachedMongoRepository(IMongoDbContext mongoDbContext,
                                      IMongoSpecificationConverter<TDocument, TDocument> mongoSpecificationConverter,
                                      ICache<TKey, TDocument> cache,
                                      Func<TDocument, TKey> keySelector,
                                      Settings settings = null)
        ...
        protected virtual void DoCreateIndeces(IMongoCollection<TDocument> collection,
        ...
        protected virtual IMongoCollection<TDocument> GetOperativeCollection() => GetCollection<TDocument>();
                                              ...
                                              if (missedKeys.Count > 0)
                                              {
                                                  using (await _lock.LockAsync(missedKeys))
                                                  {
                                                      ...
                                                      if (missedKeys.Count > 0)
                                                      {
                                                          var roots =
                                                              await
                                                                  SearchMongoAsync(new ManyIdMongoSpecification<TKey, TDocument
                                                                                       >(missedKeys.ToArray()),
                                                                                   null,
                                                                                   null);
        ...
        protected IMongoCollection<T> GetCollection<T>(string partitionKey = null) => DbContext.GetCollection<T>(partitionKey);
            ...
            else
            {
                ...
                var updates = updating.Select(a => a)
                                      .Select(d => new ReplaceOneModel<TDocument>(_filterBuilder.And(FindByKey(d.Id),
                                                                                                     _filterBuilder.Eq(a => ((IHasVersion)a)
                                                                                                                           .DataVersion,
                                                                                                                       ((IHasVersion)d)
                                                                                                                       .DataVersion - 1)),
                                                                                  d))
        ...
        private static bool IsCreating(IHasVersion document) => document.DataVersion == Constants.INITIAL_DATA_VERSION;

        private SearchResult<TDocument> SearchMongo(IMongoSpecification<TDocument> specification,
        ...
        private async Task<TDocument[]> SearchMongoAsync(IMongoSpecification<TDocument> specification,
                                                         IDomainSpecification<TDocument> domainSpecification,
                                                         SearchOptions searchOptions)
        {
            var filter = specification != null ? specification.BuildFilter(Builders<TDocument>.Filter) : Builders<TDocument>.Filter.Empty;
        ...
        private FilterDefinition<TDocument> FindByKey(TKey key) => _filterBuilder.Eq(data => data.Id, key);
