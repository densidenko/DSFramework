...
{
    public abstract class MongoSpecificationConverter<TAggregateRoot> : MongoSpecificationConverter<string, TAggregateRoot>,
                                                                        IMongoSpecificationConverter<TAggregateRoot> where TAggregateRoot : IHasId
    { }

    public abstract class MongoSpecificationConverter<TKey, TAggregateRoot> : IMongoSpecificationConverter<TAggregateRoot, TAggregateRoot>
        where TAggregateRoot : IHasId<TKey>
    {
        ...
        public IDomainSpecification<TAggregateRoot> Convert(IMongoSpecification<TAggregateRoot> source) => throw new NotImplementedException();
    }
    ...
    {
        protected MongoSpecificationConverter()
            : base(key => key)
        { }
    }
    ...
    public abstract class
        MongoSpecificationConverter<TDomainKey, TDataKey, TAggregateRoot, TAggregateRootData> : IMongoSpecificationConverter<TAggregateRoot,
            TAggregateRootData> where TAggregateRootData : IHasId<TDataKey>
    {
        ...
        public virtual IMongoSpecification<TAggregateRootData> Convert(IDomainSpecification<TAggregateRoot> source)
        {
            switch (source)
            {
                ...
                case IManyIdSpecification<TDomainKey, TAggregateRoot> manyIdSpecification:
                    return new ManyIdMongoSpecification<TDataKey, TAggregateRootData>(manyIdSpecification
                                                                                      .Ids.Select(key => _keyConverter(key))
                                                                                      .ToArray());
                case IAndDomainSpecification<TAggregateRoot> and:
