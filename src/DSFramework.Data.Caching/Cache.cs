...
namespace DSFramework.Data.Caching
{
    public class Cache<TKey, TValue> : ICache<TKey, TValue>
    {
        ...
        public Cache(ILogger logger, int expirationTimeoutMinutes, string name = null)
            : this(logger, name)
        {
        ...
        public Cache(ILogger logger,
                     int expirationTimeoutMinutes,
                     int maxCapacity,
                     string name = null)
            : this(logger, expirationTimeoutMinutes, name)
        {
        ...
        /// <returns></returns>
        public TValue Get(TKey key, Func<TKey, TValue> creator) => Get(key, (k, tags) => creator(k));
        ...
        public void UpdateValue(TKey key, TValue value, params CacheTag[] tags) => UpdateValue(key, value, null, tags);

        /// <summary>
        ///     Update value of the stored object. Doesn't prolong its lifetime. If there is no object in cache does nothing.
        /// </summary>
        public void UpdateValue(TKey key,
                                TValue value,
                                Func<TValue, TValue, bool> needUpdate,
                                params CacheTag[] tags)
        {
            lock (_lockObject)
            {
        ...
        private void RefreshKeysCount() => Observer.KeysCount(_name, _keys.Count);

        private void AddImpl(TKey key,
                             TValue value,
                             bool touch,
                             params CacheTag[] tags)
        {
        ...
        private void CleanupRoutine()
        {
            ...
            lock (_lockObject)
            {
                ...
                _logger.LogDebug($"Cleanup by time complete, items count ={_keys.Count}, removed = {removed}, maxLastTimeAccessed = {maxLastTimeAccessed.ToLocalTime()}, minLastTimeAccessed = {minLastTimeAccessed.ToLocalTime()}");
        ...
        void ICache<TKey, TValue>.Update(TKey key, TValue value) => Update(key, value);

        void ICache<TKey, TValue>.Add(TKey key, TValue value) => Add(key, value);
    }
}