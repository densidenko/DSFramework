...
namespace DSFramework.Web.Metrics.Prometheus.Mvc
{
    public class MvcActionsMetricsFilter : Attribute, IResourceFilter
    {
        private static readonly Counter _requestsCounter =
            global::Prometheus.Client.Metrics.CreateCounter("mvc_requests_total",
                                                            "MVC requests count",
                                                            "code",
                                                            "method",
                                                            "controller",
        ...
        private static readonly Counter _failedRequestsCounter =
            global::Prometheus.Client.Metrics.CreateCounter("mvc_requests_failed_total",
        ...
        private static readonly Histogram _durations = global::Prometheus.Client.Metrics.CreateHistogram("mvc_requests_duration_seconds",
        ...
        public void OnResourceExecuting(ResourceExecutingContext context) => _sw = Stopwatch.StartNew();
        ...
        public void OnResourceExecuted(ResourceExecutedContext context)
        {
            try
            {
                ...
                _durations.Labels($"{code}",
                                  method,
                                  controller,
                                  action,
                                  remoteIp,
                                  route)
                          .Observe(_sw.Elapsed.TotalSeconds);
                if (HttpHelper.IsFailed(code))
                {
                    _failedRequestsCounter.Labels($"{code}",
                                                  method,
                                                  controller,
                                                  action,
                                                  remoteIp,
